...
- **Orchestrator** (this agent): owns the baseline pointer, the strategy allocation, the promotion
  decision, the ledger, and the exit check. Never edits the experimental surface itself. Stays
  lean — it delegates all heavy work so its own context doesn't bloat across rounds.
- **Experimenter** (subagent, N per round): given one strategy and an isolated worktree, makes a
  focused change to the *mutable surface only*, commits it with a rationale, reports back. Disposable.
- **Validator** (subagent): runs the pinned measurement protocol on each candidate **and** on the
  current baseline, blind to which strategy produced which candidate. Reports the primary metric and
  the gate results. Never proposes changes.
...
Do not start looping until these are pinned down. Write them to `ratchet/config.yaml` in the repo.
...
6. **Strategy mix and search topology.** The starting allocation across strategy archetypes (see
   `references/strategy-archetypes.md`; the default barbell is a fine start), and one structural
   choice: **single-lineage** or **population**. Single-lineage is the pure ratchet — one baseline,
   promote-the-best, simplest and cheapest. **Population (island model)** maintains the top-m distinct
   lineages in parallel, lets cross-pollination breed across them, and periodically lets a strong
   island overtake a weak one. It escapes local optima far better but multiplies state and cost by
   roughly m. Default to single-lineage; switch on population when the user expects a rugged landscape
   with several viable basins. The topology details live in `references/strategy-archetypes.md`.
...
Decide how the N candidates are distributed across strategy archetypes. This is the bandit rhyme:
tilt toward archetypes that have been yielding, away from ones that haven't — but damp the tilt by
measurement noise (a noisy metric makes a hot streak look real when it's luck). Apply:
...
- **Barbell, not bell curve.** Because the ratchet clips downside, weight the extremes — cheap
  ...
  neither a high hit-rate nor a big payoff. This is Taleb, not Markowitz: you are not damping
  variance, you are buying lottery tickets whose losses are capped at zero.
- **Breathe with state.** While the metric is still improving, weight the low-risk end (cheap wins
  convert reliably). On a plateau, the cheap moves are exhausted and the escape archetypes are the
  only arms with positive expected delta — shift weight to the tail. As budget/iterations run low,
  tilt toward exploit.
- **Always seed at least one escape arm** (first-principles rewrite, subtractive/ablation, or
  cross-pollination of two prior winners) so a round on a plateau is never wasted.

Assign each candidate a concrete strategy instance, plus a short **"dead ends" list** drawn from the
ledger so agents don't re-explore what already failed, and a **diversity hint** (a distinct region of
the search space) so N agents from the same baseline don't all propose the same edit (mode collapse).

Full archetype catalog and allocation math: `references/strategy-archetypes.md`.
...
globs. Each makes its change, runs a quick local sanity check if cheap, commits with a one-paragraph
rationale, and reports its branch name and a diff summary. Keep the worktrees; the orchestrator reads
their diffs.
...
Rank eligible candidates by the **composite suite score** and take the best. Promote it **only if all**
hold:
...
promotes: baseline unchanged — and this is information, not failure. It tells the allocator to shift
toward exploration next round.

### 5. Record and check exit

Append a full round entry to the ledger (`ratchet/ledger.jsonl`): round number, baseline commit, every
candidate's strategy + diff summary + metric + gate results + disposition, the promotion decision and
its justification. Update the per-archetype yield stats. Then evaluate exit criteria. If none are met,
recurse to step 1 with the new (or unchanged) baseline.
