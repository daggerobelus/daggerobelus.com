...
Everything the loop needs to be auditable and resumable lives in a `ratchet-forum/` directory in the repo.

## `ratchet-forum/config.yaml`
...
surfaces:
  ...
  frozen:  ["bench/**", "evals/**", "ratchet-forum/**"]    # validator, data, gates, config — untouchable
...
strategy:
  active_set_size: 6         # archetypes in rotation per round; universe may be larger
  universe: [refine, tune, restructure, subtract, generative, first_principles, cross_pollinate, wildcard]
  growable: true             # add a new archetype at runtime when a winner fits none
  credit: argumentative      # model which REASONS track truth, not a per-archetype win counter

review:                      # human-in-the-loop, optional
  mode: on_demand            # autonomous | gated | on_demand
  pause_on: [challenger_flag, surprising_result, every_n_rounds]
  every_n_rounds: 5          # only if listed in pause_on
  challenger: always         # the independent agent evaluator runs every round regardless of human mode
...
The `frozen` list explicitly naming `bench/`, `evals/`, and `ratchet-forum/` is the tamper guard. If any of
...
```json
{
  ...
  "candidates": [
    ...
    { "branch": "r12-c1", "archetype": "first_principles", "instance": "rebuild abbreviation expansion from contraction grammar",
      "justification": "current expansion is lookup-based and misses unseen contractions; a grammar should generalize",
      "predicted_effect": "+0.02 composite via word_accuracy", "diff_summary": "skill/transcribe/abbrev.* +130 -84",
      "metric": { "composite": 0.908, "spread": 0.006 }, "gates": "pass", "floors": "pass",
      "reason_tracked": true, "disposition": "promoted",
  ...
  "promotion": { "branch": "r12-c1", "new_baseline": "c7e2d10",
    "justification": "0.908 vs 0.871 baseline; margin 0.037 >> 0.005σ; reason tracked; survived rerun and same-hand holdout",
    "challenger": "objected that c0's long-s fix may be the real driver; checked — c1 holds without it; objection overruled",
    "human_note": "expert confirmed grammar approach matches how she reads contractions; rescued c0 to archive" },
  "credit_update": { "first_principles": "reason predicted effect; reinforce grammar-from-structure line" },
