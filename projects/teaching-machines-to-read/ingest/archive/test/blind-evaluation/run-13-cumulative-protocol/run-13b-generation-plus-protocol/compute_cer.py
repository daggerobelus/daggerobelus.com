#!/usr/bin/env python3
"""
Compute Character Error Rate (CER) for Run 13b blind transcription evaluation.
...
def normalize_blind(text):
    ...
    # Remove header lines (title, agent info, === separators)
    lines = content.split('\n')
    clean_lines = []
    past_header = False
    for line in lines:
        if '=====' in line:
            past_header = True
            continue
        if not past_header and ('Transcription by Agent' in line or 'Brumwich MS160' in line):
            continue
...
def main():
    base = os.path.expanduser('~/Desktop/run-13b-generation-plus-protocol')
    ...
    report = []
    report.append("=" * 80)
    report.append("BLIND TRANSCRIPTION EVALUATION REPORT — RUN 13b")
    report.append("Generation Effect + Cumulative Error Protocol → Brumwich MS160")
    ...
    report.append("error protocol (Run 13). Compare to Run 4 baseline (9.30% CER,")
    ...
    for agent_num in range(1, 6):
        blind_path = os.path.join(base, f'agent-{agent_num}', 'output',
    ...
    comparisons = [
        ...
        (f"Run 13b avg (generation + protocol)", avg_cer),
        (f"Run 13b best (Agent {best_agent})", best_cer),
    ]
    ...
    diff = avg_cer - 9.30
    if diff < 0:
        report.append(f"  Run 13b avg vs previous best: {diff:+.2f}pp (IMPROVED)")
    else:
        report.append(f"  Run 13b avg vs previous best: {diff:+.2f}pp")
    diff_best = best_cer - 9.30
    if diff_best < 0:
        report.append(f"  Run 13b best vs previous best: {diff_best:+.2f}pp (IMPROVED)")
    else:
        report.append(f"  Run 13b best vs previous best: {diff_best:+.2f}pp")
    report.append("")
    ...
    # Save report
    output_path = os.path.join(base, 'evaluation', 'run-13b-evaluation-report.txt')
