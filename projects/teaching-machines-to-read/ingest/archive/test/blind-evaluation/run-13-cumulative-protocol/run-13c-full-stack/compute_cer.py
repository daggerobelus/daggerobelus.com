#!/usr/bin/env python3
"""
Compute Character Error Rate (CER) for Run 13c blind transcription evaluation.
...
def main():
    base = os.path.expanduser('~/Desktop/run-13c-full-stack')
    ...
    report = []
    report.append("=" * 80)
    report.append("BLIND TRANSCRIPTION EVALUATION REPORT — RUN 13c")
    report.append("Generation Effect + Cumulative Error Protocol + Alphabet-First → Brumwich MS160")
    ...
    comparisons = [
        ...
        (f"Run 13c avg (generation + protocol)", avg_cer),
        (f"Run 13c best (Agent {best_agent})", best_cer),
    ]
    ...
    diff = avg_cer - 9.30
    if diff < 0:
        report.append(f"  Run 13c avg vs previous best: {diff:+.2f}pp (IMPROVED)")
    else:
        report.append(f"  Run 13c avg vs previous best: {diff:+.2f}pp")
    diff_best = best_cer - 9.30
    if diff_best < 0:
        report.append(f"  Run 13c best vs previous best: {diff_best:+.2f}pp (IMPROVED)")
    else:
        report.append(f"  Run 13c best vs previous best: {diff_best:+.2f}pp")
    report.append("")
    ...
    # Save report
    output_path = os.path.join(base, 'evaluation', 'run-13c-evaluation-report.txt')
